...
# For Products
def seed_product(variable, size_array, price_array, brand, time_array, stock_array)
  zipped_array = size_array.zip(price_array)
  zipped_array2 = time_array.zip(stock_array)
  begin
    if variable.find_by(name: "#{brand}").count.equal?0
      zipped_array.zip(zipped_array2) do |np, ts|
